...
- **CRUD**: Crear (*Método HTTP POST*), Leer (*Método HTTP GET*), Actualizar (*Método HTTP PUT o PATCH*), Eliminar (*Método HTTP DELETE*).
...
- Las estimaciones de tráfico se basan en los mercados conocidos actualmente; un crecimiento rápido puede requerir un balanceo de carga o escalado vertical/horizontal.
...
- **Autenticación**: Bearer tokens para validar (autenticar) editores.
...
Esta sección contiene diagramas de pipelines, describiendo así el proceso de implementación de features y arreglo de bugs cuando estos se implementen al sistema, realizándoles pruebas cuando se quiera subir dichos cambios a GitHub, indicando hasta el flujo de integración (merge) de ramas y asignando cada una al proceso de desarrollo:
Dev branch -> Testing branch -> Main branch (Producción).
...
- **S1: Servicio de Reseñas**: Maneja la creación y validación de reseñas.
- **S2: Servicio de Contenido**: Optimizado para lectura rápida de reseñas.
- **S3: Servicio de Autenticación**: Emite y valida tokens JWT.
