...
class SecondaryWindow(QtWidgets.QMainWindow):
    def __init__(self, title, show_table=False):
        super().__init__() 
        ...
        if show_table:
            try:
                self.__createTable()  # Crea y muestra la tabla si show_table es True
            except Exception as e:
                self.__showErrorMessageBox(str(e))  # Muestra un cuadro de diálogo con el mensaje de error
        else:
            self.__createEmptyTable()  # Crea una tabla vacía si show_table es False
    ...
    def __createTable(self):
        ...
        try:
            ...
            if (type(resultDataFrame) == str):
                ...
                # Código para llenar la tabla con los datos del DataFrame
...
if __name__ == "__main__":
    ...
    window = SecondaryWindow("Ventana 2", show_table=True)  # Puedes establecer show_table a True o False según lo necesites
